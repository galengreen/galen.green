...
flowchart TB
    subgraph CMS["galen-cms app"]
        ...
        subgraph Payload["Payload CMS"]
            ...
            API[REST API]
            AdminUI[Admin UI]
            Jobs[Payload Jobs<br/>media-processing]
        end
...
- The CMS uses Payload Jobs `autoRun` to process queued media derivative jobs inside the CMS container
- Uploads store the original file immediately, then generate responsive derivatives asynchronously
- This requires the CMS to run as a long-lived server process; it is not designed for serverless execution

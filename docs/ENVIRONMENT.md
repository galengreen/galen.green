...
```mermaid
flowchart TB
    subgraph Frontend["Frontend (Vite Build)"]
        FE_ENV["VITE_PAYLOAD_URL<br/>(build-time only)"]
    end
    ...
    subgraph CMS["CMS (Payload/Next.js)"]
        ...
        CMS_OPT["Optional:<br/>GITHUB_TOKEN<br/>RESEND_API_KEY<br/>NOTIFICATION_EMAIL<br/>CRON_SECRET<br/>DEV_FRONTEND_URL"]
    end
...
# Or point to production CMS
VITE_PAYLOAD_URL=https://galen.green
```
